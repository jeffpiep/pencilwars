...
for iRoid in range(numRoids):
    roidPosX.append(random.randint(int(size[0]/6) + roidRadius, int(size[0]/6*5) - roidRadius))
    roidPosY.append(random.randint(roidRadius,size[1]-roidRadius))
deathRadius = 50
...
while done == False:
    ...
    if gameState == 0: #see if there is a winner
        ...
        moveShip = True
        #TODO: upgrade logic so the closest object struck controls the outcome
        # algorithm: test all objects and create a list of struct objects and their distance
        # determine shortest distance and object type
        # then destroy object and process accordingly
    ...
    # draw the ships
    pygame.draw.circle(screen, black, (deathPosX, deathPosY), deathRadius, 0)
    for iPlay in range(2):
        for iShip in range(numShips):
            shipID = iPlay*numShips+iShip
            if shipID == shipSelected and gameState > 0:
                currPos=(shipPosX[shipID],shipPosY[shipID])                    
                pygame.draw.circle(screen, green, currPos, shipRadius, 0)
            elif shipAlive[shipID]:
                currPos=(shipPosX[shipID],shipPosY[shipID])                    
                pygame.draw.circle(screen, playCol[iPlay], currPos, shipRadius, 0)
    # draw the asteroids
    for iRoid in range(numRoids):
        if roidAlive[iRoid]:
    ...
    if shipSelected != -1 and gameState == 0:
        if moveShip:
            pygame.draw.line(screen,green,[shipPosX[shipSelected]-flickDX,shipPosY[shipSelected]-flickDY],[shipPosX[shipSelected],shipPosY[shipSelected]],5)
        else:
            pygame.draw.line(screen,green,[shipPosX[shipSelected],shipPosY[shipSelected]],[shipPosX[shipSelected]+flickDX,shipPosY[shipSelected]+flickDY],5)
